...
from db_manager import init_db, get_db_tables, get_db_data, add_to_db
...
def index():
  try:
    ...
    l = [da, uz, ort, temp, nitrat, nwl, nitrit, ammo, awl, phos, pwl, ph, gpsx, gpsy]
    if request.args.get('send')=='send':
      init_db(l[0])
      add_to_db(l)
      
      f = open('templates/data.csv', 'a')
      f.write(';'.join([da,uz, ort, temp, nitrat, nwl, nitrit, ammo, awl, phos, pwl, ph,gpsx,gpsy])+'\n')
      f.close()
  ...
  if request.args.get('url') != None:
    ...
    return render_template('main.html', date=dat, link=link)
  else:
    return render_template('main.html', date=dat, link='static/images.jpg')
@app.route('/down')
def download():
